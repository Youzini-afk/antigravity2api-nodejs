...
export function buildConfig(jsonConfig, upstreamCfg = {}) {
  ...
  return {
    ...
    retryIntervalMs: getEnvNumber('RETRY_INTERVAL_MS') ?? (Number.isFinite(jsonConfig.other?.retryIntervalMs) ? jsonConfig.other.retryIntervalMs : DEFAULT_RETRY_INTERVAL_MS),
    retryPollTokenWithQuota: getEnvBoolean('RETRY_POLL_TOKEN_WITH_QUOTA') ?? (jsonConfig.other?.retryPollTokenWithQuota === true),
